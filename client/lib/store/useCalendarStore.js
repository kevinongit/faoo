...
const useCalendarStore = create((set) => ({
  ...
  fetchMonthlySales: async (business_number, year, month) => {
    ...
    try {
      const response = await fetch(
        "http://localhost:6100/api/dashboard/sales/month",
        {
          method: "POST",
          headers: {
  ...
  fetchComparison: async (business_number) => {
    ...
    try {
      const response = await fetch(
        "http://localhost:6100/api/dashboard/sales/comparison",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  fetchDailySales: async (business_number) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:6100/api/dashboard/sales/daily`,
        {
          method: "POST",
      ...
      if (!response.ok)
        throw new Error("매출 데이터를 불러오는 데 실패했습니다.");
