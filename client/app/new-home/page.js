    ...
    <div className="w-full px-4 py-4 text-sm text-gray-800">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4 mb-4 shadow-sm">
        ...
        <div className="text-xl font-bold">
          ₩4,850,000 <span className="text-green-500 text-sm">▲12%</span>
        </div>
        <div className="w-full h-2 mt-2 bg-gray-200 rounded-full">
          <div
            className="h-full bg-[#4A5CFB] rounded-full"
            style={{ width: "60%" }}
          ></div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4 mb-4 shadow-sm">
        ...
        <div className="grid grid-cols-7 gap-2 text-center text-sm">
          {["월", "화", "수", "목", "금", "토", "일"].map((label) => (
            <div key={label} className="text-gray-500 font-medium">
              {label}
            </div>
          ))}
            ...
            return (
              <div
                ...
                className={`py-2 rounded-xl ${
                  isToday ? "bg-[#EEF1FF]" : "bg-white"
                }`}
              >
                <div
                  className={`w-8 h-8 mx-auto rounded-full flex items-center justify-center mb-1 text-sm font-bold ${
                    isToday ? "text-[#4A5CFB]" : "text-gray-700"
                  }`}
                >
                  {dateObj.date()}
                </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4 mb-4 shadow-sm">
        <div className="grid grid-cols-4 divide-x divide-gray-200 text-xs text-center text-gray-600">
          {[
            ...
            {
              label: "연결관리",
              icon: <Link className="w-6 h-6 text-red-500" />,
            },
          ].map(({ label, icon }) => (
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4 pb-2 shadow-sm mb-4">
        <div className="flex justify-between items-center mb-3">
          ...
          <span className="text-xs text-[#4A5CFB] font-medium cursor-pointer">
            더보기
          </span>
        </div>
        <ul className="divide-y divide-gray-100">
          {[
            ...
            <li
              key={idx}
              className="flex items-center justify-between py-3 cursor-pointer hover:bg-gray-50 rounded-md px-1"
            >
              <div className="text-sm text-gray-600 leading-tight">
                ...
                {item.action && (
                  <div className="text-[11px] text-blue-500">{item.action}</div>
                )}
              </div>
